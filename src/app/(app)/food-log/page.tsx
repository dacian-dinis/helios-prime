...
import { ChevronLeft, ChevronRight, Plus, Trash2, Coffee, Sun, Moon, Cookie, Camera, Barcode, ChefHat } from "lucide-react";
...
export default function FoodLogPage() {
  ...
  return (
    <div className="pb-24 md:pb-0">
      ...
      {/* Daily Summary Bar */}
      <div className="mb-5 grid grid-cols-4 gap-2 rounded-xl border border-foreground/10 bg-foreground/[0.02] p-3">
        {[
          { label: "Calories", value: totals.calories, target: profile.dailyCalories, unit: "" },
          { label: "Protein", value: totals.protein, target: profile.protein, unit: "g" },
          { label: "Carbs", value: totals.carbs, target: profile.carbs, unit: "g" },
          { label: "Fat", value: totals.fat, target: profile.fat, unit: "g" },
        ].map((m) => (
          <div key={m.label} className="text-center">
            <p className="text-xs text-foreground/50">{m.label}</p>
            <p className="text-sm font-bold">
              {m.value}{m.unit}{" "}
              <span className="font-normal text-foreground/40">/ {m.target}{m.unit}</span>
            </p>
          </div>
        ))}
      </div>
